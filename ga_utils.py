...
def mutate_states(
    ...
    for i, (field, mutate_func) in enumerate(mutators.items()):
        ...
        # selected_new_values = jnp.where(fields_to_mutate[i], new_values, old_values)
        # TODO This is a strange construct and it should be possible with lax.select or jnp.where
        # The issue is that fields_to_mutate is (batch_size,) but new_values might be (batch_size, H, W)
        # and they must be of the same shape
        # selected_new_values = jax.lax.fori_loop(
        #     0,
        #     batch_size,
        #     lambda j, a: a.at[i].set(
        #         jax.lax.cond(
        #             fields_to_mutate[i][j], lambda: new_values[j], lambda: old_values[j]
        #         )
        #     ),
        #     new_values,
        # )
...
def _mutate_int(value, key, strength, low, high):
    range_size = high - low
    delta = jax.random.randint(key, (), -range_size, range_size + 1)
    return jnp.clip(
        value + jnp.floor(delta * strength).astype(jnp.int32), low, high - 1
    )


def _mutate_bool(value, key, strength):
    return jax.random.bernoulli(key, p=strength)


def _mutate_float(value, key, low, high, strength):
    return jnp.clip(value + jax.random.normal(key) * strength * (high - low), low, high)
