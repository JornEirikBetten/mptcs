...
from qdax_modified.custom_types import (
    Centroid,
    Descriptor,
    ExtraScores,
    Fitness,
...
class MapElitesRepertoire(flax.struct.PyTreeNode):
    ...
    Args:
        ...
        rngs: an array that contains the random keys of the solutions in each cell
            of the repertoire, ordered by centroids. The array shape is (num_centroids, shape_of_rng).
    """
    ...
    def save(self, path: str = "./") -> None:
        ...
        jnp.save(path + "rngs.npy", self.rngs)
    ...
    def sample(self, random_key: RNGKey, num_samples: int) -> Tuple[Genotype, RNGKey]:
        ...
        random_key, subkey = jax.random.split(random_key)
        sample_indices = jax.random.choice(subkey, jnp.arange(self.centroids.shape[0]), shape=(num_samples,), p=p)
        sample_genotypes = self.genotypes[sample_indices]
        sample_rngs = self.rngs[sample_indices]
        return sample_genotypes, sample_rngs, random_key
    ...
    def add(
        ...
        batch_of_rngs: Optional[RNGKey] = None,
    ) -> MapElitesRepertoire:
        """
        ...
        Args:
            ...
            batch_of_rngs: unused tree that contains the extra_scores of
        ...
        new_rngs = self.rngs.at[batch_of_indices.squeeze(axis=-1)].set(
            batch_of_rngs
        )
    ...
    def init(
        ...
        new_repertoire = repertoire.add(genotypes, descriptors, fitnesses, rngs)
    ...
    def init_default(
        ...
        default_rngs = jax.tree_util.tree_map(
