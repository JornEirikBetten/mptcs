...
def build_mutate_fn(mutators: Dict[str, Callable], observe_fn: Callable, mutation_prob: float, mutation_strength: float) -> Callable:
    def mutate_fn(state: pgx.State, key: jax.random.PRNGKey) -> Tuple[pgx.State, jnp.ndarray]:
        mutated_state, mutations = mutate_states(state, key, mutators, observe_fn, mutation_prob, mutation_strength)
        return mutated_state, mutations
    return mutate_fn
