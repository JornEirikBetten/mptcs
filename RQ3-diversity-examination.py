...
evaluate_and_collect = build_execution_fn(evaluator_fn, NUM_EVAL_POLICIES, 25)
...
for generation in range(1, NUM_GENERATIONS + 1): 
    ...
    #print("Number of TCs in topk: ", jnp.sum(topk_storage.fitnesses > 0))
    topk_test_cases = topk_candidates 
    # minimal_pats candidate selection 
    ...
    if generation % EVALUATION_INTERVAL == 0: 
        # evaluate minimal_pats test suite 
        num_unique_observations, mean_failure_rate, fail_counter, mptcs_validation_archive = evaluate_and_collect(test_params_stacked, mptcs_test_suite)
        print(f"MPTCS: mfr={mean_failure_rate}, n_unique_obs={num_unique_observations}")
        print(fail_counter)
        
        # evaluate topk test suite 
        num_unique_observations, mean_failure_rate, fail_counter, topk_validation_archive = evaluate_and_collect(test_params_stacked, topk_storage)
        print(f"Top-k: mfr={mean_failure_rate}, n_unique_obs={num_unique_observations}")
        print(fail_counter)

        save_path = os.getcwd() + f"/results/diversity/{env_name}/experiment_{experiment_name}/generation_{generation}/"
        os.makedirs(save_path, exist_ok=True)
        mptcs_save_path = save_path + "mptcs/"
        topk_save_path = save_path + "topk/"
        os.makedirs(mptcs_save_path, exist_ok=True)
        os.makedirs(topk_save_path, exist_ok=True)
        mptcs_test_suite.save(mptcs_save_path)
        topk_storage.save(topk_save_path)
        utils.plot_archive(mptcs_validation_archive, 
                           generation, 
                           save_path + f"mptcs_validation.pdf", 
                           "failure rate", xlabel, ylabel, xtick_labels, ytick_labels, ticks, shape=CENTROIDS_SHAPE)
        utils.plot_archive(topk_validation_archive, 
                           generation, 
                           save_path + f"topk_validation.pdf", 
                           "failure rate", xlabel, ylabel, xtick_labels, ytick_labels, ticks, shape=CENTROIDS_SHAPE)
